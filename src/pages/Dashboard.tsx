...
import admin from "../../public/6.png"
    ...
    const unsubscribe = onSnapshot(collection(db, 'notification'), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          const newMessage = change.doc.data().mesage
          if (!notification.includes(newMessage)) {
            toast({ variant: "destructive", title: "Security Alert", description: newMessage });
            setNotification(prev => [...prev, newMessage]);
            localStorage.setItem("notification", JSON.stringify([...notification, newMessage]));
          }
        }
      });
    });
            ...
            <nav className="grid items-start px-4 text-sm font-medium">
              <Link
                className= {cn("flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50",
                 location.pathname == "/dashboard" && "bg-gray-100 text-gray-900 " 
                )}
                to="/dashboard"
              ...
              <Link
                className= {cn("flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50",
                location.pathname == "/dashboard/users" && "bg-gray-100 text-gray-900 "
               )}
               to="users"
              >
              ...
              <Link
                className= {cn("flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50",
                location.pathname == "/dashboard/rooms" && "bg-gray-100 text-gray-900 " 
               )}
               to="rooms"
              >
              ...
              <Link
                 className= {cn("flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50",
                 location.pathname == "/dashboard/logs" && "bg-gray-100 text-gray-900 " 
                )}
                to="logs"
              >
